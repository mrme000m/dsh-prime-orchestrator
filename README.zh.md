...
- **Web 界面**：Prime 舰队侧栏（Web 界面右侧，侧栏底部开关），实时展示委托/会话/事件流；以及 设置 → Prime 编排 分区。点进任意运行中的 prime-agent 会话可查看实时对话、发送提示（文本或斜杠命令）、控制当前轮（中止、队列、目标、模型切换），并给出 `prime-agent attach` 命令以便在 TUI 中接管 —— 网页视图与 TUI 共享同一个守护会话。
