...
/** Shared CSS Modules plugin, parameterized by the plugin id stamped on style tags. */
function CSS_MODULES_PLUGIN(bundleId: string) {
  return {
    ...
    async load(this: { addWatchFile: (file: string) => void }, virtualId: string) {
      ...
      return [
        ...
        `const tagId = ${JSON.stringify(`${bundleId}/${basename(fileId)}`)};`,
...
/** Shared bundle purity gate over the module-table external list. */
function CLIENT_PURITY_PLUGIN() {
  return {
...
export default defineConfig([
  ...
  {
    // The patched stock ui-layout browser bundle: registered under the
    // STOCK module id, so the boot-manifest rewrite (host side) swaps the
    // layout entry's URL to this artifact and the browser composes the
    // four-column shell (adds the 'prime' column) without touching the dsh
    // installation. Built from the stock 0.1.1-rc.2 sources plus the prime
    // column patch; externals match the stock layout bundle exactly.
    entry: { 'layout-override': 'layout-override/src/client/index.ts' },
    outDir: 'lib',
    format: ['cjs'],
    platform: 'browser',
    target: ['es2022'],
    ...
    plugins: [CSS_MODULES_PLUGIN('@deepseek-ai/dsh-client-ui-layout'), CLIENT_PURITY_PLUGIN()],
    outputOptions: {
      entryFileNames: 'layout-override.js',
  ...
  },
  {
    ...
    noExternal: (id: string) => (CLIENT_EXTERNALS.includes(id) ? undefined : true),
    plugins: [CSS_MODULES_PLUGIN(PLUGIN_ID), CLIENT_PURITY_PLUGIN()],
